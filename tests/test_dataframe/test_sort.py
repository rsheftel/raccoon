...
def test_sort_index():
    ...
    assert_frame_equal(
        df, rc.DataFrame({"a": [2, 3, 1], "b": [5, 6, 4]}, columns=["a", "b"], index=[8, 9, 10], sort=False)
    )
...
def test_sort_multi_index():
    df = rc.DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6]}, columns=["a", "b"], index=[(10, "c"), (10, "a"), (10, "b")], sort=False
    )
    ...
    assert_frame_equal(
        df,
        rc.DataFrame(
            {"a": [2, 3, 1], "b": [5, 6, 4]}, columns=["a", "b"], index=[(10, "a"), (10, "b"), (10, "c")], sort=False
        ),
    )
...
def test_sort_column_w_key():
    ...
    assert_frame_equal(
        df, rc.DataFrame({"a": [4, 3, 2, 1], "b": ["d", "c", "b", "a"]}, columns=["a", "b"], index=[11, 10, 9, 8])
    )

    # a key function that turns evens into an odds into a negative number
    def even_to_neg(i):
        return i * -1 if i % 2 == 0 else i

    df.sort_columns("a", key=even_to_neg)
    assert_frame_equal(
        df, rc.DataFrame({"a": [4, 2, 1, 3], "b": ["d", "b", "a", "c"]}, columns=["a", "b"], index=[11, 9, 8, 10])
    )

    # with key and reverse
    df.sort_columns("a", key=even_to_neg, reverse=True)
    assert_frame_equal(
        df, rc.DataFrame({"a": [3, 1, 2, 4], "b": ["c", "a", "b", "d"]}, columns=["a", "b"], index=[10, 8, 9, 11])
    )
