...
def test_get_cell():
    actual = rc.DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6], "c": [7, 8, 9]}, index=[10, 11, 13], columns=["a", "b", "c"], sort=False
    )
...
def test_get_cell_sorted():
    actual = rc.DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6], "c": [7, 8, 9]}, index=[10, 11, 13], columns=["a", "b", "c"], sort=True
    )
...
def test_get_columns():
    ...
    expected = rc.DataFrame(
        {"a": [4], "b": [7], "c": [None]}, index=[99], columns=["a", "b", "c"], index_name="start_10", sort=False
    )
    actual = df.get_columns(99)
    assert_frame_equal(actual, expected)

    # specific columns
    expected = rc.DataFrame({"a": [4], "c": [None]}, index=[99], columns=["a", "c"], index_name="start_10", sort=False)
    ...
    expected = rc.DataFrame({"c": [8], "b": [5]}, index=[11], columns=["c", "b"], index_name="start_10", sort=False)
...
def test_get_columns_sorted():
    ...
    expected = rc.DataFrame({"a": [4], "c": [None]}, index=[99], columns=["a", "c"], index_name="start_10", sort=True)
    actual = df.get(99, ["a", "c"])
    assert_frame_equal(actual, expected)
    ...
    # columns out of order
    expected = rc.DataFrame({"c": [8], "b": [5]}, index=[11], columns=["c", "b"], index_name="start_10", sort=True)
    actual = df.get(11, ["c", "b"])
    assert_frame_equal(actual, expected)
...
def test_get_matrix():
    ...
    expected = rc.DataFrame(
        {"b": [4, 6], "d": [10, 12]}, index=["x", "z"], columns=["b", "d"], index_name="letters", sort=False
    )
    ...
    # columns out of order
    expected = rc.DataFrame(
        {"d": [10, 12], "c": [7, 9]}, index=["x", "z"], columns=["d", "c"], index_name="letters", sort=False
    )
...
def test_get_matrix_sorted():
    df = rc.DataFrame(
        {"a": [2, 1, 3], "b": [5, 4, 6], "c": [8, 7, 9], "d": [11, 10, 12]},
        index=["y", "x", "z"],
        columns=["a", "b", "c", "d"],
        index_name="letters",
        sort=True,
    ...
    expected = rc.DataFrame(
        {"b": [4, 6], "d": [10, 12]}, index=["x", "z"], columns=["b", "d"], index_name="letters", sort=True
    )
    actual = df.get(["x", "z"], ["b", "d"])
    assert_frame_equal(actual, expected)
    ...
    # columns out of order
    expected = rc.DataFrame(
        {"d": [10, 12], "c": [7, 9]}, index=["x", "z"], columns=["d", "c"], index_name="letters", sort=True
    )
    actual = df.get(["x", "z"], ["d", "c"])
    assert_frame_equal(actual, expected)
...
def test_get_square_brackets():
    df = rc.DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6], "c": [7, 8, 9], "d": [10, 11, 12]}, columns=["a", "b", "c", "d"], sort=False
    )
    ...
    assert_frame_equal(
        df[["a", "b", "c"]],
        rc.DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": [7, 8, 9]}, columns=["a", "b", "c"], sort=False),
    )
    ...
    assert_frame_equal(
        df[[1, 2], ["a", "d"]], rc.DataFrame({"a": [2, 3], "d": [11, 12]}, columns=["a", "d"], index=[1, 2], sort=False)
    )

    assert_frame_equal(
        df[[2, 0], ["d", "a"]], rc.DataFrame({"d": [12, 10], "a": [3, 1]}, columns=["d", "a"], index=[2, 0], sort=False)
    )


def test_get_square_brackets_sorted():
    df = rc.DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6], "c": [7, 8, 9], "d": [10, 11, 12]}, columns=["a", "b", "c", "d"], sort=True
    )
    ...
    assert_frame_equal(
        df[["a", "b", "c"]],
        rc.DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": [7, 8, 9]}, columns=["a", "b", "c"], sort=True),
    )
    ...
    # df[[1, 2], ['a', 'd']] -- get indexes = [4, 5], columns = ['a', 'b']
    assert_frame_equal(
        df[[1, 2], ["a", "d"]], rc.DataFrame({"a": [2, 3], "d": [11, 12]}, columns=["a", "d"], index=[1, 2], sort=True)
    )

    assert_frame_equal(
        df[[2, 0], ["d", "a"]], rc.DataFrame({"d": [10, 12], "a": [1, 3]}, columns=["d", "a"], index=[0, 2], sort=True)
    )
...
def test_get_slicer():
    ...
    assert_frame_equal(
        df[0:1, ["c", "d"]], rc.DataFrame({"c": [7, 8], "d": [10, 11]}, columns=["c", "d"], index=[0, 1], sort=False)
    )

    assert_frame_equal(
        df[0:1, ["d", "c"]], rc.DataFrame({"d": [10, 11], "c": [7, 8]}, columns=["d", "c"], index=[0, 1], sort=False)
    )
...
def test_get_slicer_sorted():
    ...
    # df[0:1, ['c', 'd']] -- get slice from index 0 to 1, columns ['c', 'd']
    assert_frame_equal(
        df[0:1, ["c", "d"]], rc.DataFrame({"c": [7, 8], "d": [10, 11]}, columns=["c", "d"], index=[0, 1], sort=True)
    )

    assert_frame_equal(
        df[0:1, ["d", "c"]], rc.DataFrame({"d": [10, 11], "c": [7, 8]}, columns=["d", "c"], index=[0, 1], sort=True)
    )
