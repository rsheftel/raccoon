...
def test_print():
    df = rc.DataFrame(
        {"a": [1, 2, 3], "b": [1.0, 2.55, 3.1], "c": ["first", "second", None]},
        columns=["b", "c", "a"],
...
def test_json():
    ...
    with pytest.raises(AttributeError) as e:
        ...
        assert (
            e == "AttributeError: the JSON has a dropin : <class 'blist.blist'> : "
    ...
    with pytest.raises(AttributeError) as e:
        ...
        assert (
            e == "AttributeError: the supplied dropin parameter: <class 'list'> : does not match the value"
...
def test_columns_blist():
    actual = rc.DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]}, index=["a", "b", "c"], columns=["b", "a"], dropin=blist)
...
def test_sort_index():
    df = rc.DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]}, columns=["a", "b"], index=[10, 8, 9], sort=False, dropin=blist)

    df.sort_index()
    assert isinstance(df.index, blist)
    assert_frame_equal(
        df,
        rc.DataFrame({"a": [2, 3, 1], "b": [5, 6, 4]}, columns=["a", "b"], index=[8, 9, 10], sort=False, dropin=blist),
    )
...
def test_sort_column():
    ...
    assert isinstance(df.index, blist)
    assert_frame_equal(
        df, rc.DataFrame({"a": [1, 2, 3], "b": ["c", "a", "b"]}, columns=["a", "b"], index=[8, 10, 9], dropin=blist)
    )

    df.sort_columns("a", reverse=True)
    assert isinstance(df.index, blist)
    assert_frame_equal(
        df, rc.DataFrame({"a": [3, 2, 1], "b": ["b", "a", "c"]}, columns=["a", "b"], index=[9, 10, 8], dropin=blist)
    )
